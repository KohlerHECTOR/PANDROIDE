...
class NormalPolicy(GenericNet):
    ...
    def get_weights_as_numpy(self):
        weights=np.zeros((self.s_size+1)*self.h1_size + (self.h1_size+1)*self.h2_size + (self.h2_size+1)*2)
        fc1_w = self.fc1.weight.data.detach().clone().numpy().flatten()
        fc1_b = self.fc1.bias.data.detach().clone().numpy().flatten()
        fc2_w = self.fc2.weight.data.detach().clone().numpy().flatten()
        fc2_b = self.fc2.bias.data.detach().clone().numpy().flatten()
        fc_mu_w = self.fc_mu.weight.data.detach().clone().numpy().flatten()
        fc_mu_b = self.fc_mu.bias.data.detach().clone().numpy().flatten()
        fc_std_w = self.fc_std.weight.data.detach().clone().numpy().flatten()
        fc_std_b = self.fc_std.bias.data.detach().clone().numpy().flatten()
        return np.concatenate((fc1_w,fc1_b,fc2_w,fc2_b,fc_mu_w,fc_mu_b,fc_std_w,fc_std_b))
    ...
    def forward(self, state):
        ...
        mu = self.fc_mu(state)
        std = self.fc_std(state) # Note : Can be set to have better results with pg
        return mu, std
