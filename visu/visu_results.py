...
def exploit_angles_global_full(params) -> None:
    ...
    study = params.gradients
    for i in range(len(study)):
        plot_data(path + "/angle_" + params.experiment+study[i] + '_' + params.env_name + '.txt', "scalar product "+params.experiment)
...
def exploit_distance_solo(params) -> None:
    ...
    study = params.gradients
    for i in range(len(study)):
        plot_data(path + "/distance_" + params.experiment+study[i] + '_' + params.env_name + '.txt', "distance "+params.experiment)
        # plot_data(path + "/reward_" + 'evo_pg'+study[i] + '_' + params.env_name + '.txt', "reward evo_pg")
    plt.title(params.env_name)
    plt.xlabel("Episodes")
    plt.ylabel("distance between policies at episode x and x+1 ")
